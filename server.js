...
mongoose
  .connect(
    "mongodb+srv://admin:admin271*@cluster.wtbgcs2.mongodb.net/?retryWrites=true&w=majority&appName=Cluster",
    {
...
app.post("/posts", async (req, res) => {
  ...
  try {
    ...
    if (!canSubmit(user.lastPostTimestamps)) {
      return res
        .status(429)
        .send("Post submission limit reached. Please try again later.");
    }
...
app.post("/posts/:postId/comments", async (req, res) => {
  ...
  try {
    ...
    const commentLimitEntry = user.commentTimestamps.find(
      (entry) => entry.post.toString() === postId
    );
    ...
    if (commentLimitEntry) {
      ...
      const recentTimestamps = commentLimitEntry.timestamps.filter(
      ...
      if (recentTimestamps.length >= 4) {
        return res
          .status(429)
          .send("Comment limit reached for today on this post.");
      }
...
app.post("/sendFeedback", async (req, res) => {
  ...
  try {
    ...
    if (!canSubmit(user.lastFeedbackTimestamps)) {
      return res
        .status(429)
        .send("Feedback submission limit reached. Please try again later.");
    }
...
app.post("/posts/:postId/report", async (req, res) => {
  ...
  try {
    ...
    await post.save();
    res.json({
      message: `Report ${action === "add" ? "added" : "removed"} successfully.`,
      action: action,
    });
  } catch (error) {
...
app.delete("/posts/:postId", async (req, res) => {
  ...
  try {
    ...
    if (!user || !user.isAdmin) {
      return res
        .status(403)
        .send("Unauthorized: Only admins can delete posts.");
    }
    ...
    // Update the user document to remove the post ID from the posts array
    await User.updateOne({ email: userEmail }, { $pull: { posts: postId } });
